...
// Same shape as tmd_obj_export.cpp's material grouping - kept as its own
// small copy here since the two exporters otherwise share nothing (glTF
// needs per-primitive-group vertex buffers, OBJ needs per-face `usemtl`
// text), and this is genuinely the whole of what differs.
...
bool SameMaterial(const ExportMaterial& a, const ExportMaterial& b) {
    ...
    for (int i = 0; i < n; i++) {
        if (a.color[i][0] != b.color[i][0] || a.color[i][1] != b.color[i][1] || a.color[i][2] != b.color[i][2]) return false;
    }
    return true;
}

struct Vec3f { float x, y, z; };
Vec3f ToExportSpace(const tmd::TMD_Vertex& v) { return { (float)v.x, -(float)v.y, (float)v.z }; }
Vec3f ToExportSpace(const tmd::TMD_Normal& n) { return { (float)n.x, -(float)n.y, (float)n.z }; }
